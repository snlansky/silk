...
impl PublicKey {
    ...
    pub fn into_protobuf_encoding(self) -> Vec<u8> {
        ...
        let public_key = match self {
            PublicKey::Ed25519(key) =>
                silk_proto::PublicKey {
                    r#type: silk_proto::KeyType::Ed25519 as i32,
                    data: key.encode().to_vec()
                },
            #[cfg(not(any(target_os = "emscripten", target_os = "unknown")))]
            PublicKey::Rsa(key) =>
                silk_proto::PublicKey {
                    r#type: silk_proto::KeyType::Rsa as i32,
                    data: key.encode_x509()
                },
            #[cfg(feature = "secp256k1")]
            PublicKey::Secp256k1(key) =>
                silk_proto::PublicKey {
                    r#type: silk_proto::KeyType::Secp256k1 as i32,
                    data: key.encode().to_vec()
                }
        };
    ...
    pub fn from_protobuf_encoding(bytes: &[u8]) -> Result<PublicKey, DecodingError> {
        ...
        #[allow(unused_mut)] // Due to conditional compilation.
            let mut pubkey = silk_proto::PublicKey::decode(bytes)
            .map_err(|e| DecodingError::new("Protobuf").source(e))?;

        let key_type = silk_proto::KeyType::from_i32(pubkey.r#type)
            .ok_or_else(|| DecodingError::new(format!("unknown key type: {}", pubkey.r#type)))?;

        match key_type {
            silk_proto::KeyType::Ed25519 => {
                ed25519::PublicKey::decode(&pubkey.data).map(PublicKey::Ed25519)
            },
            #[cfg(not(any(target_os = "emscripten", target_os = "unknown")))]
            silk_proto::KeyType::Rsa => {
    ...
    pub fn into_peer_id(self) -> String {
        unimplemented!()
    }
}