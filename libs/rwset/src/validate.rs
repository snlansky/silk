...
use crate::key::{derive_hashed_data_ns, PubAndHashUpdates};
use silk_proto::*;
use statedb::{are_same, Height, UpdateBatch, VersionedDB};
        ...
        if let (Some(header), Some(data)) = (block.header, block.data) {
            ...
            for (index, proto_msg) in data.data.iter().enumerate() {
                ...
                let tx_header = utils::proto::unmarshal::<Proposal>(&proposal.proposal_bytes)?
                    .header
                    .ok_or(from_str("transaction header is null"))?;

                let resp = tx
                    .response
                    .get(0)
                    .ok_or(from_str("transaction proposal response list is null"))?;
